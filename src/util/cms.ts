...
export function postList(options?: ButterListOptions): Promise<AxiosResponse<ButterListResponse<BlogPost[]>>> {
  return ButterApi.post.list(options) as Promise<AxiosResponse<ButterListResponse<BlogPost[]>>>;
}

export function getPost(slug: string): Promise<AxiosResponse<ButterRetrieveResponse>> {
  return ButterApi.post.retrieve(slug) as Promise<AxiosResponse<ButterRetrieveResponse>>;
}

export function searchPosts(query: string): Promise<AxiosResponse<ButterListResponse<BlogPost[]>>> {
