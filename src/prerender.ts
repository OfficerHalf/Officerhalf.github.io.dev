...
import {
  ListResponse,
  BattlemapPage
} from './app/blog/interfaces/butter.interface';
...
function getPage<T>(
  ...
  return axios.get<ListResponse<T>>(endpoint, {
    params: { page },
...
async function getAllPages<T>(endpoint: string): Promise<T[]> {
  ...
  while (next_page !== null) {
    const response: AxiosResponse<ListResponse<T>> = await getPage<T>(
      endpoint,
      next_page
    );
...
async function getMapList() {
  const battlemapList = await getAllPages<BattlemapPage>(
    `${butter}/pages/battlemap`
  );
  ...
  await promises.writeFile(
    './src/.battlemaplist.json',
    JSON.stringify(battlemapList),
    'utf-8'
  );
  await promises.writeFile(
    './src/.battlemapmap.json',
    JSON.stringify(battlemapMap),
    'utf-8'
  );
}
